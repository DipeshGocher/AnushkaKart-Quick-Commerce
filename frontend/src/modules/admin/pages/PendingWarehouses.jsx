...
const PendingWarehouses = () => {
    ...
    const fetchPendingSellers = async () => {
        setIsLoading(true);
        try {
            const response = await adminApi.getPendingWarehouses({ q: searchTerm || undefined });
            ...
            toast.error(error.response?.data?.message || 'Failed to load warehouse applications');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const filteredSellers = useMemo(() => {
        return pendingSellers.filter(s =>
            String(s.warehouseName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const reviewDocuments = useMemo(() => {
        ...
        if (Array.isArray(viewingWarehouse.documentFiles) && viewingWarehouse.documentFiles.length) {
            return viewingWarehouse.documentFiles;
        }

        return (viewingWarehouse.documents || []).map((label, index) => ({
            key: `legacy-${index}`,
            label,
            url: '',
            fileName: label,
    ...
    const handleApprove = async (id) => {
        ...
        try {
            await adminApi.approveWarehouse(id);
            ...
            toast.error(error.response?.data?.message || 'Failed to approve warehouse');
        } finally {
    ...
    const handleReject = async (id) => {
        if (window.confirm('Are you sure you want to reject this application?')) {
            ...
            try {
                ...
                await adminApi.rejectWarehouse(id, { reason });
                setIsReviewModalOpen(false);
                setViewingWarehouse(null);
                toast.success('Warehouse application rejected');
                await fetchPendingSellers();
            } catch (error) {
                console.error('Failed to reject warehouse', error);
                toast.error(error.response?.data?.message || 'Failed to reject warehouse');
            } finally {
                setIsProcessing(false);
            }
        }
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div>
                    <h1 className="ds-h1 flex items-center gap-2">
                        Warehouse Pending Approvals
                        <Badge variant="warning" className="admin-tiny px-1.5 py-0 font-bold animate-pulse">Action Required</Badge>
                    </h1>
                    <p className="ds-description mt-0.5">Check new warehouse applications before they can start selling.</p>
                </div>
                                ...
                                <tr>
                                    <td colSpan="4" className="px-6 py-20 text-center">
                                        <div className="flex flex-col items-center justify-center gap-3">
                                            ...
                                            <p className="text-slate-500 font-bold text-sm">Loading warehouse applications...</p>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tr key={s.id} className="hover:bg-slate-50/30 transition-colors group">
                                    <td className="px-6 py-5 align-middle">
                                        <div
                                            ...
                                            onClick={() => navigate(`/admin/warehouses/active/${s.id}`)}
                                        >
                                            ...
                                            <div>
                                                <p className="text-sm font-bold text-slate-900 group-hover/name:text-primary transition-colors">{s.warehouseName}</p>
                                    ...
                                    <td className="px-6 py-5 text-right align-middle">
                                        <div className="flex items-center justify-end gap-3 h-full">
                                            ...
                                            <button
                                                onClick={() => { setViewingWarehouse(s); setIsReviewModalOpen(true); }}
                                ...
                                <tr>
                                    <td colSpan="4" className="px-6 py-20 text-center">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-slate-500 font-bold text-sm">All caught up! No pending warehouse applications.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
                                    ...
                                    <div className="lg:col-span-4 bg-slate-50 p-4 border-r border-slate-100">
                                        <div className="flex justify-between items-start mb-8">
                                            <div className="h-20 w-20 rounded-xl bg-white shadow-xl flex items-center justify-center ds-stat-large font-bold text-primary border-4 border-white">
                                                {(viewingWarehouse.warehouseName || 'S').charAt(0)}
                                            </div>
                                            <button
                                                onClick={() => setIsReviewModalOpen(false)}
                                        ...
                                        <div className="space-y-6">
                                            <div>
                                                <h3 className="ds-h2 leading-tight">{viewingWarehouse.warehouseName}</h3>
                                                <p className="text-xs font-bold text-primary mt-1 uppercase tracking-widest">{viewingWarehouse.category || 'General'} PARTNER</p>
                                            </div>

                                            <div className="space-y-4">
                                                <div className="flex items-center gap-3">
                                                    <HiOutlineBuildingOffice2 className="h-4 w-4 text-slate-400" />
                                                    <span className="text-xs font-bold text-slate-700">{viewingWarehouse.ownerName}</span>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    <HiOutlineEnvelope className="h-4 w-4 text-slate-400" />
                                                    <span className="text-xs font-semibold text-slate-500">{viewingWarehouse.email}</span>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    <HiOutlinePhone className="h-4 w-4 text-slate-400" />
                                                    <span className="text-xs font-bold text-slate-700">{viewingWarehouse.phone}</span>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    <HiOutlineMapPin className="h-4 w-4 text-slate-400" />
                                                    <span className="text-xs font-semibold text-slate-500">{viewingWarehouse.location}</span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="ds-section-spacing">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {reviewDocuments.length > 0 ? reviewDocuments.map((doc) => (
                                                    ...
                                                    <div className="md:col-span-2 rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-center">
                                                        <p className="text-sm font-bold text-slate-500">No documents were submitted with this warehouse application.</p>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex items-center gap-4 pt-6">
                                                <button
                                                    ...
                                                    onClick={() => handleReject(viewingWarehouse.id)}
                                                ...
                                                {reviewDocuments.length > 0 && (
                                                    <button
                                                        disabled={isProcessing}
                                                        onClick={() => handleApprove(viewingWarehouse.id)}
