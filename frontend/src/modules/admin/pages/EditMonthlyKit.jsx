...
import {
    ...
    Tag, Image as ImageIcon, Sparkles, FolderOpen, Layers, Package, ShoppingBasket
} from 'lucide-react';
...
const EditMonthlyKit = () => {
                ...
                try {
                    ...
                    if (kit) {
                        setFormData({
                            ...
                            price: kit.price || '',
                            salePrice: kit.salePrice || kit.price || '',
                            stock: kit.stock || '',
                            lowStockAlert: kit.lowStockAlert || 5,
                            brand: kit.brand || '',
                            sku: kit.sku || '',
                            weight: kit.weight || '',
                            tags: kit.tags ? kit.tags.join(', ') : '',
                            categoryId: kit.categoryId?._id || kit.categoryId || '',
                            ...
                            includedItems: kit.includedItems && kit.includedItems.length > 0 ? kit.includedItems.map((v, i) => ({ ...v, id: v._id || i })) : [
                                { id: Date.now(), name: '', quantity: '' }
                            ]
                        });
                    }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            data.append("includedItems", JSON.stringify(validItems));
    ...
    const tabs = [
        ...
        { id: 'items', label: 'Basket Items', icon: ShoppingBasket },
        ...
        <div className="max-w-6xl mx-auto space-y-6 pb-12 p-4 md:p-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-black text-slate-900">{id ? "Edit Monthly Kit" : "Create Monthly Kit"}</h1>
                        <p className="text-xs font-medium text-slate-500">{id ? "Update existing subscription basket" : "Add a new subscription basket"}</p>
                    </div>
                </div>
                <div className="flex gap-3">
                    ...
                    >
                        ...
                        {loading ? 'Submitting...' : (id ? 'Save Changes' : 'Submit for Approval')}
                    </button>
                </div>
            </div>
                    ...
                    {activeTab === 'items' && (
                        <div className="space-y-6 animate-in fade-in duration-300">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h4 className="text-sm font-bold text-slate-900">Basket Items</h4>
                                    <p className="text-xs text-slate-500 font-medium">List the specific products included in this basket</p>
                                </div>
                                <button
                                    type="button"
                            ...
                            <div className="space-y-3">
                                {(formData.includedItems || []).map((item, index) => (
                                    <div key={item.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center gap-4 group relative">
                                        <div className="flex-1 space-y-1">
                                            <label className="text-xs font-bold text-slate-600 uppercase tracking-widest ml-1">Item Name</label>
                                            <input
                                                value={item.name}
                                                onChange={(e) => {
                                        ...
                                        <div className="flex-1 space-y-1">
                                            <label className="text-xs font-bold text-slate-600 uppercase tracking-widest ml-1">Quantity</label>
                                            <input
                                                value={item.quantity}
                                                onChange={(e) => {
                                                    const newItems = [...formData.includedItems];
