...
import warehouseAnimation from "../../../assets/INSTANT_6.json";
...
const Auth = () => {
  ...
  const [signupStep, setSignupStep] = useState(() => {
    const saved = sessionStorage.getItem("warehouse_signupStep");
  ...
  const handleBlur = async (e) => {
    ...
    if ((name === "email" && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) || 
      ...
      try {
        const res = await warehouseApi.checkExists({ [name]: value });
  ...
  const handleSendVerificationOtp = async (field) => {
    ...
    try {
      await warehouseApi.sendVerificationOtp(getVerificationPayload(field));
      updateVerificationState(field, {
        isSending: false,
        isOtpVisible: true,
        status: "otp-sent",
  ...
  const handleVerifyOtp = async (field) => {
    ...
    try {
      const response = await warehouseApi.verifyVerificationOtp({
  ...
  const handleSendResetOtp = async (e) => {
    ...
    try {
      await warehouseApi.sendResetOtp({ channel: resetData.channel, rawValue: resetData.rawValue });
  ...
  const handleVerifyResetOtp = async (e) => {
    ...
    try {
      ...
      const response = await warehouseApi.verifyResetOtp(payload);
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      await warehouseApi.resetPassword({
        channel: resetData.channel,
        rawValue: resetData.rawValue,
    ...
    try {
      ...
      const response = isLogin
        ...
        : await (() => {
          ...
          return warehouseApi.signup(signupPayload);
        })();
      ...
      if (isLogin) {
        ...
        navigate("/warehouse/pending-approval", {
          replace: true,
          state: {
  ...
  return (
    <SignInCard2
      title={isLogin ? "Welcome Back" : `Warehouse Signup (${signupStep}/3)`}
      subtitle={
        isLogin
