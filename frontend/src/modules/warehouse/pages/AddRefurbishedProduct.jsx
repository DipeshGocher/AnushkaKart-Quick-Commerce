...
import { REFURBISHED_HIGHLIGHT_PRESETS } from "../../seller/pages/AddRefurbishedProduct";
...
const AddRefurbishedProduct = () => {
  ...
  const makeSku = (name, index = 1) => {
    const prefix =
      String(name || "")
        ...
        .replace(/[^a-z0-9]/g, "")
        .slice(0, 5) || "item";
    return `wh-ref-${prefix}-${String(index).padStart(3, "0")}`;
  };
  ...
  React.useEffect(() => {
    const fetchCats = async () => {
      try {
        const res = await warehouseApi.getCategoryTree({ catalogType: "refurbished" });
        if (res.data.success) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(response?.data?.message || "Refurbished product added to warehouse catalog!");
              ...
              <div className="bg-gradient-to-r from-orange-500/10 via-amber-500/10 to-orange-500/5 p-5 rounded-2xl border border-orange-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">Specify device condition grade, battery health %, warranty and included box items</p>
                  </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-2 duration-300">
              <div className="space-y-1.5 flex flex-col">
                ...
                <input
                  ...
                  placeholder="e.g. Apple MacBook Air M1 (256GB SSD) - Space Gray"
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1.5 flex flex-col">
                  ...
                  <input
                    ...
                    placeholder="e.g. Apple, Samsung, Lenovo, HP"
                  />
                </div>
                <div className="space-y-1.5 flex flex-col">
                  <label className="text-[10px] sm:text-xs font-bold text-slate-600 uppercase tracking-widest ml-1">
          ...
          {modalTab === "category" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-2 duration-300">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1.5 flex flex-col">
                  <label className="text-[10px] sm:text-xs font-bold text-slate-600 uppercase tracking-widest ml-1">
                    Main Category Group <span className="text-rose-500">*</span>
                  </label>
                  <select
